...
async function getData() {
    ...
    for (var i = 0, l = user_sites.length; i < l; i++) {
        if (user_sites[i].signurl.search(host_url)!==-1) {
            ...
            data.site_cookie = ptCookie; // cookie为空应该false，不发送
			data.site_include = ""
			if(temp.signin_enable == true){data.site_include += "Q";}
			if(temp.rss_enable == true){data.site_include += "D";}
			if(temp.brush_enable == true){data.site_include += "S";} // 若未设定站点用途，则使用默认
